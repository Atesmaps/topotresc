...
class RenderThread:
    ...
    def render_tile(self, x, y, z):
        ...
        print(f"{threading.current_thread().name} rendering block: {z + self.zoom_correction} {x} {y}")
        ...
        for i in range(self.block_size):
            ...
            tile_folder = f"{TILE_DIR}{z + self.zoom_correction}/{xx}/"
            ...
            for j in range(self.block_size):
                ...
                tile_uri = f"{tile_folder}{yy}.png"
                tile = img.crop(
                    (i * OSM_TILE_SIZE, j * OSM_TILE_SIZE, (i + 1) * OSM_TILE_SIZE, (j + 1) * OSM_TILE_SIZE))
...
def render_tiles(bbox, minZoom=1, maxZoom=17, block_size=1, num_threads=NUM_THREADS):
    print(f"render_tiles({bbox}, {minZoom}, {maxZoom})")
    ...
    for i in range(num_threads):
        ...
        render_thread.start()
        print(f"Started render thread {render_thread.name!r}")
