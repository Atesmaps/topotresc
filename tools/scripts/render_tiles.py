...
def render_tiles(map_polygon, zoom, image_size, folder, areas_done):
    tiles = []
    # Bounds return floats. Convert to and correct to the 256 closer tile.
    (minx, miny, maxx, maxy) = [(int(p) & ~(int('FF',16))) for p in map_polygon.bounds]
    if areas_done:
        plt.plot(*areas_done.exterior.xy)
    ...
    for tile in tiles:
        plt.plot(*tile.exterior.xy, c='green')
    if areas_done:
        plt.plot(*areas_done.exterior.xy, c="red")
    plt.savefig(f"{TILES_DIR}tiles_{zoom}_{image_size}.png")
...
def render_map(geojson_map, zooms):
    ...
    for block_size in [16]:
        ...
        for zoom in range(zooms[0], zooms[1]+1):
            ...
            map_polygon = geom.Polygon([gprj.fromLLtoPixel(point_ll, zoom_corrected) for point_ll in map_ll])
            ...
            plt.clf()
            plt.plot( *map_polygon.exterior.xy)
            if zoom in areas_done and areas_done[zoom]:
                plt.plot(*areas_done[zoom].exterior.xy, c='red')
            plt.savefig(TILES_DIR + "area_done_%s_%s.png" % (zoom, image_size))
        for _ in workers:
            queue.put("Done")
        while not queue.empty():
            print(f" *-----------------> {time.time() - start_time:.2f}s.  \t#  Queue size: {queue.qsize()}")
            time.sleep(10)
